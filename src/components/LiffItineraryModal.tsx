    ...
    return (
        <div className="fixed inset-0 z-50 bg-white dark:bg-[#111111] flex flex-col h-full w-full animate-slideUp">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6 pb-20 text-[var(--text-main)]">
                    ...
                    <div className="space-y-2">
                        {itineraryItems.map((item, idx) => (
                            <div
                                ...
                                className={`flex items-center gap-3 p-4 bg-white dark:bg-[#1f2c34] border ${draggedIdx === idx ? 'border-[var(--line-primary)] opacity-50 shadow-md' : 'border-gray-100 dark:border-gray-800'} rounded-xl shadow-sm cursor-grab active:cursor-grabbing hover:border-gray-300 transition-all`}
                            >
                                <svg className="text-gray-400" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="9" cy="12" r="1"></circle><circle cx="9" cy="5" r="1"></circle><circle cx="9" cy="19" r="1"></circle><circle cx="15" cy="12" r="1"></circle><circle cx="15" cy="5" r="1"></circle><circle cx="15" cy="19" r="1"></circle></svg>
                                <div className="flex-1 font-medium">{item.text}</div>
                                {item.type === 'pending' && (
                                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400">待確認</span>
                                )}
                            </div>
                        ))}
                    </div>
                ...
                {/* Budget */}
                <div className="bg-gray-50 dark:bg-[#1f2c34]/50 p-4 rounded-xl border border-gray-100 dark:border-gray-800">
                    <h3 className="font-bold text-base mb-2 flex items-center gap-2" style={{ color: "#FFaa00" }}>
                    ...
                    <p className="font-medium text-[15px] pl-6 mb-0">{summary.estimatedBudget}</p>
                </div>

                {/* Recommendations */}
                {summary.bookingCards && summary.bookingCards.length > 0 && (
                    <div>
                        <h3 className="font-bold text-base mb-3 flex items-center gap-2" style={{ color: "var(--line-primary)" }}>
